...
|             Clusters                                                                      |   `Clusters()`       |                                  `problem` `transformer` `verbose` `algorithm` `n_clusters` `random_seed` `n_init` `max_iter` `algorithmMethod` `covariance_type` `ensamMethod` `bandwidth` `kernel` `metric` `RBFMethod` `zoom`                                  |
