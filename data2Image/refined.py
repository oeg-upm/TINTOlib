...
class REFINED:
    ...
    def __saveSupervised(self,classValue,i,folder,matrix_a):
        ...
        #plt.imshow(matrix_a.reshape(2, 2), cmap='viridis')
        plt.imshow(matrix_a.reshape(2, 2), cmap='viridis')
        plt.axis('off')
    ...
    def __saveRegressionOrRegression(self,i,folder,matrix_a):
        extension = 'png'  # eps o pdf
        subfolder = "images"
        ...
        plt.imshow(matrix_a.reshape(4,4), cmap='viridis')
        plt.axis('off')
        plt.savefig(fname=route_complete, bbox_inches='tight', pad_inches=0)
    ...
    def __saveImages(self,gene_names,coords,map_in_int, X, Y, nn, folder):
        ...
        for i in range(len(X_REFINED_MDS)):
            if self.problem == "supervised":
                route=self.__saveSupervised(Y[i], i, folder, X_REFINED_MDS[i])
                imagesRoutesArr.append(route)
            elif self.problem == "unsupervised" or self.problem == "regression" :
                route = self.__saveRegressionOrRegression(i, folder, X_REFINED_MDS[i])
                imagesRoutesArr.append(route)
            else:
        ...
        if self.problem == "supervised" :
            ...
            regressionCSV.to_csv(folder + "/supervised.csv", index=False)
        elif self.problem == "unsupervised":
            data = {'images': imagesRoutesArr}
            regressionCSV = pd.DataFrame(data=data)
            regressionCSV.to_csv(folder + "/unsupervised.csv", index=False)
    ...
    def __hillClimbing(self,desc, dist, img,folder):
        """
        Desc: genes names
        ...
        if my_rank == 0:
            ...
            pd.Series(corr_evol).to_csv(folder+"\\REFINED_evolve.csv")
    ...
    def __trainingAlg(self, X, Y, data, folder='img_train/'):
        """
        ...
        Img = Assign_features_to_pixels(eq_xy, nn,verbose=self.verbose)  # Img is the none-overlapping coordinates generated by MDS
        #print(Img)
        # %% To be saved for hill climbing
        """Desc = X.columns.tolist();
        Desc = Desc[2:]  # Drug descriptors name"""
        ...
        #print(Euc_Dist)
        #print(Desc)
        #Dist = pd.DataFrame(data=Euc_Dist, columns=Desc,index=Desc)
        Dist = pd.DataFrame(data=Euc_Dist)
        gene_names, coords, map_in_int=self.__hillClimbing(Desc, Dist, Img,folder)

        self.__saveImages(gene_names, coords, map_in_int,X,Y,nn,folder)
    ...
    def generateImages(self,data, folder):
        ...
        # Training
        self.__trainingAlg(X, Y,data ,folder=folder)
